...
1. 先用远程桌面软件对对远程计算机进行一个管理(前提是该远程计算机也安装了虚拟机)

2. 点击编辑

3. 选项——>共享虚拟机——>选择共享虚拟机存放的位置

4. 在把虚拟机拉到共享虚拟机的位置上，让这个虚拟机成为共享虚拟机

   ![](Vmware虚拟机.assets/img-8.png)

5. 如何连接到这台共享虚拟机呢? 只需要点击 “连接到服务器”，输入该共享虚拟机所在的计算器的ip、账号以及密码即可

   ![](Vmware虚拟机.assets/img-9.png)

   ![](Vmware虚拟机.assets/img-10.png)

   

   **注意:不是共享的虚拟机是不可以远程连接的**

## 第十二章、创建虚拟机模板，克隆新系统

- 克隆新系统的定义以及注意事项：

  1. 要将需要克隆的系统先做一个快照，然后已要克隆系统的硬盘为基础克隆到另外的一个硬盘上；

  2. 一个系统可以克隆出多个系统，而这些克隆出来的系统都是用的是快照系统（被克隆系统）的硬盘资源，有效率的节省了计算机的储存空间；

  3. 一旦被克隆系统被删除，则所有已他为基础的克隆出来的系统也会被删除；

  4. 克隆出来的系统名称与IP地址与原生系统保持一致；

  5. 原生系统中存在着sId，而克隆出来的sId都是一样的，sId又是唯一表示计算机的，所以不能够把克隆出来的多个系统区分开来，所以可以有以下步骤去解决这个问题：

     - 把克隆出来的每个系统都运行一下**sysprep**命令，然后在把该系统关闭在重新启动，则系统会有有一个分配的sId，而这个sId每个都是不一样的。

     - 在原生系统上先运行一下**sysprep**命令，然后关机做快照，接着启动系统，然后克隆出来的系统的sId都是不一样的。

       ![image-20210327155639324](Vmware虚拟机.assets/image-20210327155639324.png)

  6. 计算机在运行的时候是不能进行系统克隆的
